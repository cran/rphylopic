...
#'   silhouette.
#' @param tip `character`. The tip labels against which to add the silhouettes.
...
#' @param width,relWidth `numeric`. The width of each silhouette, in the plot
#'   coordinate system (`width`) or relative to the size of the plotting area
#'   (`relWidth`). If "NULL" and `height` is specified, the width is determined
#'   by the aspect ratio of the original image. One of height and width must be
#'   "NULL".
#' @param padding,relPadding `numeric`. Horizontal padding for each silhouette
#'   from its respective x value, in the plot coordinate system (`padding`) or
