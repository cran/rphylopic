...
#'
#' @param x \code{numeric or character}. Either the x coordinate to be used to
#'   position the legend or a keyword accepted by [legend()] such as "topleft",
#'   "topright", "bottomleft", and "bottomright".
#' @param y \code{numeric}. The y coordinate to be used to position the legend.
#'   Can be `NULL` (default) if using keywords in `x`.
#' @param legend \code{character}. A character vector of the labels to appear in
#'   the legend.
#' @param ysize `r lifecycle::badge("deprecated")` use the `height` argument
#'   instead.
...
#' @details This function can be used to add PhyloPic silhouettes as a legend to
#'   a base R plot. Arguments available in [legend()] can be used and passed via
#'   `...`. Note that not all arguments in [legend()] are compatible with
#'   [add_phylopic_legend()]. These include arguments for modifying lines (e.g.
#'   `lty`, `lwd`, `seg.len`), points (e.g. `pch`, `pt.lwd`), and shading (e.g.
#'   `angle` and `density`). This is due to [add_phylopic_legend()] using
#'   [add_phylopic_base()] to generate the legend symbols. However, arguments
#'   for adjusting the legend appearance such as text (e.g. `cex`), legend box
#'   (e.g. `bg`), and color (e.g. `border`) are compatible.
